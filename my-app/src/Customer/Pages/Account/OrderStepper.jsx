...
// const OrderStepper = ({ orderStatus, currentStep }) => {
      ...
      {statusStep.map((step, index) => (
        <div key={index} className="flex px-4">
          <div className="flex flex-col items-center">
            ...
            {statusStep.length - 1 !== index && (
              <div
                className={`border h-20 w-[2px] ${index < currentStep ? 'bg-primary-color' : 'bg-gray-300'}`}
              />
            )}
          </div>
          <div className="ml-2 w-full">
            <div
              className={`${
                step.value === orderStatus ? 'bg-primary-color text-white' : ''
